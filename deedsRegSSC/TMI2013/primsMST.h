/* Image-driven minimum-spanning-tree calcuation using Prim's algorithm.
 Average run-time should be of n*log(n) complexity.						平均运行时间，最坏时间应该是O(n^2)
...
void primsGraph(float* im1,int* ordered,int* parents,int step1){
	...
	int dx[6]={-1,1,0,0,0,0};								//6邻域的坐标
	int dy[6]={0,0,-1,1,0,0};
	int dz[6]={0,0,0,0,-1,1};
	int xx,yy,zz,xx2,yy2,zz2;
	//calculate edge-weights based on SAD of groups of voxels (for each control-point)	  对于每个控制点计算边的权重（在构造MST时需要用到）
	for(int k=0;k<o;k++){	//---------------------------------------------------------控制点A：6邻域像素和控制点B：6邻域像素-->对应的灰度差之和即为边权重。
		for(int j=0;j<n;j++){
			for(int i=0;i<m;i++){
