  ...
  //  int maxlevel=3; //number of resolution levels
   ...
   // int label_hw[]={6,4,2};
    ...
    for(int i=0;i<maxlevel;i++){
        scale_factor[i]=maxlevel-i;			 //scale_factor[0]=3; scale_factor[1]=2; scale_factor[2]=1;
    }
    //scaling of multi-resolution levels (in voxels)

    printf("SETTINGS: sigma: %f, kernel: %d\n",sigma,kernel);
     //*******************************************************以上是参数设置，包含输入fixed，moving，movingseg，输出outputstem，方框大小kernel，高斯sigma，多级框架maxlevel，位移搜索空间label_hw,以及分割和对称标识符
    ...
    //global image dimensions    确定图像的维度
    ...
    float* flowx=new float[sz*3];
	  ...
	  //创建了一个sz*sz*sz的三维向量ux，vx，wx而对应于这个的是逆变换uxi，vxi，wxi
    ...
    for(int level=0;level<maxlevel;level++){
        ...
        int hw=label_hw[level];			// int label_hw[]={6,4,2};		  ---->第一次取hw=6，允许大位移

		int sparse = scale_factor[level];	  //scale_factor[]={3,2,1};

        int r=kernel;					   //r means 取多少点作为一个Node的半径 kernel=2

        int len=hw*2+1; int len3=pow(hw*2+1,3);		   //hw是搜索半径，len表示搜索的直径，len3是3维直径

        float h1=0.05;
        int m1=m/sparse; int n1=n/sparse; int o1=o/sparse;	 //根据比例因子稀疏图像
        int sz1=m1*n1*o1;
        
        float* costvol=new float[sz1*len3];			//一共sz1个像素点，每个像素点的可能位移都是len3种-->costvol表示整幅图所有像素点各自的cost代价
