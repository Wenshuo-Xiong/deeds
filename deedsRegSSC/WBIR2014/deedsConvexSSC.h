...
int deeds(char* fixedin,char* movingin,char* movingsegin,char* outputstem,int kernel,float sigma,int maxlevel,int* label_hw,bool segment,bool symmetric) {
    ...
    float thetas[]={0.003,0.01,0.03,0.1,0.3,1};//thetas for 5 coupling iterations		论文中的乘数： theta0
    ...
    for(int level=0;level<maxlevel;level++){
        ...
        int sparse=scale_factor[level];	//{3，2，1}
        int r=kernel;
        ...
        float* costvol=new float[sz1*len3];		 //保存每个像素点的代价值（x,y,z)三个坐标维度
        float *warped1=new float[m*n*o];		 //扭曲的图像
